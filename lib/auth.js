...
export const authOptions = {
  ...
  callbacks: {
    ...
    async jwt({ token, user, account, trigger, session }) {
      console.log("JWT Callback - User:", user ? user.email : "No user", "Token:", token.email);
      if (user) {
        token.id = user.id;
      ...
      if (token.email && token.email !== "admin@gmail.com") {
        const dbUser = await getUserByEmail(token.email);
        if (dbUser) {
          token.id = dbUser.id;
          token.onboardingCompleted = dbUser.onboardingCompleted;
          token.role = "user"; // Ensure DB users are always 'user' role unless we add role to DB
        }
      }
